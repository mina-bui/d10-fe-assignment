/**
 * @file
 * Provides UI/UX progressive enhancements on Mytheme's theme settings by
...
((Drupal, settings, once) => {
  const colorSchemeOptions = settings.mytheme.colorSchemes;
  ...
  function synchronizeInputs(changedInput, inputToSync) {
    ...
    changedInput.setAttribute("data-mytheme-custom-color", changedInput.value);
    inputToSync.setAttribute("data-mytheme-custom-color", changedInput.value);
  ...
  function setColorScheme({ target }) {
    ...
    if (selectedColorScheme) {
      ...
      document
        .querySelectorAll(`input[data-mytheme-custom-color]`)
        .forEach((input) => {
          input.value = input.getAttribute("data-mytheme-custom-color");
        });
    }
  }
  ...
  /**
   * Initializes Mytheme theme-settings color picker.
  ...
  function initColorPicker(textInput) {
    ...
    colorInput.setAttribute(
      "data-mytheme-custom-color",
      textInput.getAttribute("data-mytheme-custom-color")
    );
  ...
  /**
   * Mytheme Color Picker behavior.
   *
   * @type {Drupal~behavior}
   * @prop {Drupal~behaviorAttach} attach
   *   Initializes color picker fields.
   */
  Drupal.behaviors.mythemeColorPicker = {
    attach: () => {
      ...
      const colorTextInputs = once(
        "mytheme-color-picker",
        '[data-drupal-selector="mytheme-color-picker"] input[type="text"]'
      );
